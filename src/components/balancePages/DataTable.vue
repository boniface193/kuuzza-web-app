      ...
      <table class="custom-table">
        ...
        <thead class="custom-thead">
          <tr>
            ...
            <th v-if="showShipping === true">Shipping</th>
          </tr>
        </thead>
        <!-- tables body -->
          ...
          >
              ...
              <span class="with-checkbox">
                ...
                >
                  <router-link
                    :to="{
                      name:
                        item.order_type !== 'OPEN_SELLING'
                          ? header.routeNameOrder
                          : header.routeNameOpenOrder,
            ...
            <td v-if="showShipping === true">
              <v-select
                ...
                dense
                outlined
                @change="updateShipping(item[`${itemKey}`])"
              >
              </v-select>
            </td>
...
export default {
  name: "DataTable",
  components: { selectBtn, deleteIcon, removeIcon, editIcon },
  data: function () {
    return {
      updateItem: "select status",
      ...
      currentSort: "",
      modifier: 1,
      shippingItem: ["SHIPPED", "DELIVERED"],
    };
  },
  props: [
    "items",
  ...
  methods: {
    updateShipping(params) {
      this.updatedShippingStatus = this.updateItem;
      if (this.updateItem == "SHIPPED" || this.updateItem == "DELIVERED") {
        this.updateItem = "Update Shipping Status";
      }
      this.$emit("getItemID", params);
      this.$emit("checkForUpdate", this.updatedShippingStatus);
    },
    // sort data
    sort(data) {
      if (data == undefined) {
